...
fn test_write_and_read_mcap_episode() {
    ...
    let video_ch = writer
        ...
        .unwrap();
    let obs_ch = writer
        .add_channel("/observation/leader/joint_position", SchemaType::JointStates, bfbs)
        .unwrap();
    let action_ch = writer
        .add_channel("/action/follower/joint_position", SchemaType::JointStates, bfbs)
        .unwrap();
    ...
    for i in 0..10u64 {
        ...
        writer.write_message(video_ch, us_to_ns(ts_us), &fb_data).unwrap();
    }
    ...
    for i in 0..30u64 {
        ...
        writer.write_message(obs_ch, us_to_ns(ts_us), &fb_data).unwrap();
    }
    ...
    for i in 0..30u64 {
        ...
        let fb_data = encode::encode_joint_states(ts_us, &values, None);
        writer.write_message(action_ch, us_to_ns(ts_us), &fb_data).unwrap();
    }
    ...
    assert_eq!(messages.len(), 70, "Expected 70 messages, got {}", messages.len());

    // Verify channel topics
    let topics: std::collections::HashSet<&str> = messages
        .iter()
    ...
    let first_video = messages.iter().find(|m| m.channel.topic == "/camera/cam_left/video").unwrap();
    let video = flatbuffers::root::<rollio_episode_mcap::fb::foxglove::CompressedVideo>(&first_video.data).unwrap();
