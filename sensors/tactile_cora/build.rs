//! Build script for `tactile-cora`. Mirrors `sensors/imu_cora/build.rs`:
...
fn main() {
    ...
    if target_os != "linux" {
        println!(
            "cargo:warning=tactile-cora: target_os={} is not 'linux'; \
    ...
    if sdk_root.is_none() && !system_cora_available() {
        println!(
            "cargo:warning=tactile-cora: Cora SDK not found via CORA_SDK_ROOT, \
