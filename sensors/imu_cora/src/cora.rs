...
/// Run a standalone DDS discovery sweep on `domain_id` for `wait_ms`
/// milliseconds. Returns the unique `(topic, type)` pairs that publishers
/// announced during the window. Independent of `Bridge` lifecycle — uses
/// its own ephemeral DomainParticipant, so safe to call before `Bridge::new`.
pub fn discover_topics(
    domain_id: i32,
    participant_name: &str,
    wait_ms: u32,
) -> Result<Vec<(String, String)>> {
    let name = CString::new(participant_name)?;
    let collected: Mutex<Vec<(String, String)>> = Mutex::new(Vec::new());

    extern "C" fn trampoline(
        topic: *const std::os::raw::c_char,
        ty: *const std::os::raw::c_char,
        user: *mut c_void,
    ) {
        if topic.is_null() || ty.is_null() || user.is_null() {
            return;
        }
        let topic = unsafe { std::ffi::CStr::from_ptr(topic) }
            .to_string_lossy()
            .into_owned();
        let ty = unsafe { std::ffi::CStr::from_ptr(ty) }
            .to_string_lossy()
            .into_owned();
        let vec = unsafe { &*(user as *const Mutex<Vec<(String, String)>>) };
        vec.lock().push((topic, ty));
    }

    let user_ptr = &collected as *const _ as *mut c_void;
    let rc = unsafe {
        ffi::cora_bridge_discover_topics(
            domain_id,
            name.as_ptr(),
            wait_ms,
            Some(trampoline),
