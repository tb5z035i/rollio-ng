...
fn main() {
    ...
    for f in [
        "vendor/stubs.c",
        "src/ffi.c",
        "vendor/include/hb_media_codec.h",
        "vendor/include/hb_media_basic_types.h",
        "vendor/include/hb_media_error.h",
    ] {
...
fn build_stub_so(vendor_dir: &Path, out_dir: &Path) {
    ...
    let compiler = cc::Build::new().file(&stubs_c).get_compiler();
    ...
    let s = compile
        .status()
        .unwrap_or_else(|e| panic!("stub compile: {}: {e}", cc_path.display()));
    ...
    let s = link
        .status()
        .unwrap_or_else(|e| panic!("stub link: {}: {e}", cc_path.display()));
