...
// ---------------------------------------------------------------------------
// EagerCoraSdkNalAssembler — see h264_annexb.hpp for the contract.
// TODO(coracam-temp): delete once Cora SDK emits AU-granular H264 samples.
// ---------------------------------------------------------------------------

void EagerCoraSdkNalAssembler::emit_pending() {
    if (pending_.empty()) {
        return;
    }
    ready_buf_ = std::move(pending_);
...
void EagerCoraSdkNalAssembler::emit_single(const uint8_t* data, std::size_t size,
...
auto EagerCoraSdkNalAssembler::feed(const uint8_t* data, std::size_t size, uint64_t timestamp_us)
    -> bool {
    if (!has_annexb_start_code(data, size) || size < 4U) {
        return false;
    }
    ...
    // Only single-NAL samples are expected from the Cora SDK today. If a
    // multi-NAL sample shows up (e.g. mid-deployment SDK upgrade), assume
    // it is already a complete AU and ship it as-is.
    if (offsets.size() > 1U) {
        // Drop any in-flight param-set buffer — it would be incoherent next
        // to a complete AU coming through.
        if (!pending_.empty()) {
            pending_.clear();
            pending_ts_us_ = 0;
            pps_offset_ = 0;
        ...
        emit_single(data, size, timestamp_us);
        return true;
    }

    const uint8_t nal_type = data[offsets.front()] & 0x1FU;
    ...
    switch (state_) {
        case State::Idle:
            if (nal_type == kNalTypeSps) {
                ...
                ++counters_.orphan_pps;
            } else if (nal_type == kNalTypeIdr) {
                ++counters_.orphan_idr;
            } else if (nal_type == kNalTypeNonIdr) {
                emit_single(data, size, timestamp_us);
                return true;
            } else {
                ++counters_.unknown_nal;
            }
            return false;

        case State::GotSps:
            if (nal_type == kNalTypeSps) {
                ++counters_.param_set_resets;
                pending_.clear();
                append();
                // state stays GotSps
            } else if (nal_type == kNalTypePps) {
                pps_offset_ = pending_.size();
                append();
                state_ = State::GotSpsPps;
            } else if (nal_type == kNalTypeIdr) {
                // IDR without PPS — drop the partial buffer + the IDR.
                ++counters_.orphan_idr;
                pending_.clear();
                pending_ts_us_ = 0;
                state_ = State::Idle;
            } else if (nal_type == kNalTypeNonIdr) {
                ++counters_.slice_breaks_param_set;
                pending_.clear();
                pending_ts_us_ = 0;
                state_ = State::Idle;
                emit_single(data, size, timestamp_us);
        ...
        case State::GotSpsPps:
            if (nal_type == kNalTypeSps) {
                ...
                // Replace previous PPS with this one (keep SPS prefix).
                pending_.resize(pps_offset_);
                append();
                // state stays GotSpsPps
            } else if (nal_type == kNalTypeIdr) {
                append();
                emit_pending();
                return true;
...
auto EagerCoraSdkNalAssembler::flush() -> bool {
    if (pending_.empty()) {
        return ready_;
    }
    if (ready_) {
        // A ready AU is still pending take(); keep pending_ for next round.
        return true;
    }
    emit_pending();
    return ready_;
