//! rollio-device-gripper-cora: subscribes to a Cora `sensor_msgs/JointState`
//! topic and republishes a single named joint as `JointVector15` on the rollio
//! iceoryx2 bus. dof=1 only; see `design/cora-device-drivers.md`.
...
const DRIVER_NAME: &str = "gripper-cora";
...
#[derive(Debug, Parser)]
#[command(name = "rollio-device-gripper-cora")]
#[command(about = "Cora sensor_msgs/JointState (dof=1) → iceoryx2 JointVector15 passthrough")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
