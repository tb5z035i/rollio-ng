//! `[devices.extra]` and `[devices.channels.extra]` shapes for gripper-cora.
...
#[derive(Debug, Clone)]
pub struct GripperChannelExtra {
    pub cora_topic: String,
    pub cora_qos: Qos,
    pub joint_name: String,
}

impl GripperChannelExtra {
    pub fn parse(
        ...
        let joint_name = match channel.extra.get("joint_name") {
            Some(toml::Value::String(s)) if !s.is_empty() => s.clone(),
            Some(toml::Value::String(_)) => {
                return Err(ConfigError::ChannelExtraType {
                    device: device.name.clone(),
