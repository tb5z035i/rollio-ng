...
fn main() {
    let base_url = std::env::var("DATALOOP_BASE_URL")
        .expect("set DATALOOP_BASE_URL env var");
    let token = std::env::var("DATALOOP_TOKEN")
        .expect("set DATALOOP_TOKEN env var");
    let project_id = std::env::var("PROJECT_ID")
        .expect("set PROJECT_ID env var");
    ...
    fs::write(&test_file, "hello from rollio-storage-dataloop smoke test\n")
