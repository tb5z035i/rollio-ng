...
use rollio_types::config::{
    BinaryDeviceConfig, ChannelPairingConfig, ChannelStateKind, CollectionMode, DeviceType,
    EncoderCodec, MappingStrategy, ProjectConfig, RobotCommandKind, RobotMode, RobotStateKind,
};
...
/// `setup_toggle_advanced_pipeline_logs` flips the
/// `[runtime].advanced_pipeline_logs` flag and surfaces the new value via
/// the serialized session config. Toggling twice returns to the initial
/// state. Pre-configuring this at setup time avoids forcing operators to
/// hand-edit the saved TOML to opt into verbose coracam pipeline logs.
#[test]
fn toggle_advanced_pipeline_logs_flips_runtime_flag() {
    let mut session = setup_session(&[camera_discovery("cam0")]);
    let initial = session.config.runtime.advanced_pipeline_logs;

    let changed = session
        .toggle_advanced_pipeline_logs()
        .expect("toggle should succeed on a valid config");
    assert!(changed);
    assert_eq!(session.config.runtime.advanced_pipeline_logs, !initial);
