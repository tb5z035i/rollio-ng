...
use rollio_types::config::{
    BinaryDeviceConfig, ChannelPairingConfig, ChannelStateKind, DeviceChannelConfigV2, DeviceType,
    MappingStrategy, RobotCommandKind, RobotMode, RobotStateKind,
};
...
pub(super) fn ensure_channel_publishes_state(
    ...
    if !channel
        .publish_states
        .contains(&ChannelStateKind::from(state))
    {
        channel.publish_states.push(ChannelStateKind::from(state));
    }
}
