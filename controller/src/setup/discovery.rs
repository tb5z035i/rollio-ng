...
use rollio_types::config::{
    BinaryDeviceConfig, CameraChannelProfile, ChannelStateKind, CollectionMode,
    DeviceChannelConfigV2, DeviceType, ProjectConfig, RobotMode, RobotStateKind, SensorStateKind,
};
            ...
            device.channels.iter().filter_map(|channel| {
                ...
                let supported_states = if device_type == DeviceType::Robot {
                    runtime_meta
                        ...
                        .and_then(|meta| meta.as_robot())
                        .map(|info| info.supported_states.clone())
                        .unwrap_or_else(|| {
                ...
                let runtime_robot_info = runtime_meta_for_channel.and_then(|meta| meta.as_robot());
                let supported_commands = runtime_robot_info
                    .map(|info| info.supported_commands.clone())
                    .unwrap_or_default();
                let direct_joint_compatibility = runtime_robot_info
                    .map(|info| info.direct_joint_compatibility.clone())
                    .unwrap_or_default();
