...
use rollio_types::config::{
    ...
    ChannelRecordConfig, ChannelStateKind, ChromaSubsampling, DeviceChannelConfigV2, DeviceType,
...
impl SetupSession {
    ...
    pub(super) fn add_pseudo_robot(&mut self) -> Result<bool, Box<dyn Error>> {
        ...
        let channel = DeviceChannelConfigV2 {
            ...
            dof: Some(6),
            publish_states: vec![
                ChannelStateKind::from(RobotStateKind::JointPosition),
                ChannelStateKind::from(RobotStateKind::JointVelocity),
                ChannelStateKind::from(RobotStateKind::JointEffort),
            ],
            recorded_states: vec![
                ChannelStateKind::from(RobotStateKind::JointPosition),
                ChannelStateKind::from(RobotStateKind::JointVelocity),
    ...
    pub(super) fn add_command_device(&mut self) -> Result<bool, Box<dyn Error>> {
        ...
        let channel = DeviceChannelConfigV2 {
            ...
            dof: Some(6),
            publish_states: vec![ChannelStateKind::from(RobotStateKind::JointPosition)],
            recorded_states: Vec::new(),
            control_frequency_hz: Some(60.0),
            sample_rate_hz: None,
            profile: None,
